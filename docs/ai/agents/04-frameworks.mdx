---
id: 04-frameworks
title: 4. Frameworks & SDK
sidebar_label: 4. Frameworks & SDK
description: AI Agent Frameworks & SDK - LangChain, LangGraph, OpenAI Agents SDK, Claude Agent SDK, Google Agent SDK, Spring AI
---
...
graph TB
    subgraph Python["Python Ecosystem"]
        ...
        AG[AutoGen<br/>→ MAF 1.0]
        CR[CrewAI<br/>Role-playing]
        OA[OpenAI SDK<br/>Sandbox Agents]
    end
...
> - **Zerostack**（HN, May 17）: 受 Unix 哲学启发的纯 Rust 编码 Agent，以可组合、最小化为设计理念，在 HN 上获得 518 点关注，代表了轻量级 Agent 架构的新方向。[GitHub](https://crates.io/crates/zerostack/1.0.0)
...
| Feature | Spring AI | LangGraph | Microsoft Agent Framework | OpenAI Agents SDK |
|---------|-----------|-----------|-----------------|---------|
...
| Concept | Description |
|---------|-------------|
| **Agents** | 带有指令（instructions）和工具（tools）的 LLM 封装，是基本的执行单元 |
| **Handoffs** | Agent 间的任务转移机制，支持将对话路由到最合适的 Agent |
| **Guardrails** | 输入/输出的安全检查，支持自定义验证规则 |
| **Tracing** | 内置运行追踪，支持可视化调试和性能分析 |
...
| Feature | Description |
|---------|-------------|
| **声明式 Agent 定义** | 通过配置和注解定义 Agent 行为，减少样板代码 |
| **内置工具集成** | 原生支持 Google Search、Code Execution、Vertex AI 等 |
| **多 Agent 编排** | 支持多 Agent 协作和层级式任务分配 |
| **评估框架** | 内置 Agent 行为评估和基准测试工具 |
...
| Feature | Description |
|---------|-------------|
| **Tool Use** | 原生支持 Claude 的 tool_use 功能，定义 JSON Schema 工具 |
| **Multi-turn Conversation** | 自动管理多轮对话上下文和工具调用循环 |
| **Streaming** | 支持实时流式输出，包括 thinking 和 tool_use 事件 |
| **Guardrails** | 输入/输出验证，可自定义安全检查规则 |
...
> **提示**：Claude Agent SDK 的核心是 Anthropic Messages API 的 tool_use 能力。结合 MCP 协议可以快速构建具有丰富工具的 Agent 系统。详见 [MCP Module](/docs/ai/mcp/)。
...
| **Microsoft Stack** | Microsoft Agent Framework (replaces Semantic Kernel / AutoGen) |
| **GPT-based Agents** | OpenAI Agents SDK |
| **Role-based Teams** | CrewAI |
| **Gemini/Google Cloud** | Google ADK |
| **Claude-native Agents** | Claude Agent SDK |
| **企业级团队 Agent 平台** | Runtime（YC P26） |
...
> 来源：[Anthropic Code with Claude 2026](https://www.mindstudio.ai/blog/code-with-claude-2026-new-agent-features/)、[Google I/O 2026](https://apnews.com/article/google-io-gemini-developers-conference-a984e6756032dc4af260f8fa27e8f4a9)

**MOSS: Agent 自演化系统**（arXiv, May 2026）: 论文 [2605.22794](https://arxiv.org/abs/2605.22794) 提出 MOSS（Source-Level Rewriting）框架，解决自主 Agent 系统部署后静态不变的核心问题。MOSS 能从用户交互中学习，通过源码级别重写自动修复反复出现的错误，实现 Agent 系统的持续自演化。

**DeltaBox: Agent 沙盒快速检查点**（arXiv, May 2026）: 论文 [2605.22781](https://arxiv.org/abs/2605.22781) 提出毫秒级沙盒检查点/回滚机制，支持 LLM Agent 的高频状态探索（如测试时树搜索和强化学习），相比传统 C/R 方案实现数量级性能提升。

**HuggingFace: Specialization Beats Scale**（May 2026）: HuggingFace 博文指出，在 AI 采购决策中，专业化小模型在特定任务上往往优于通用大模型。这一发现对 Agent 框架选型具有重要参考价值——不是所有 Agent 任务都需要最大参数量的模型。

**HuggingFace Agent 术语表**（May 2026）: HuggingFace 发布 [Agent 术语规范](https://huggingface.co/blog/agent-glossary)，旨在解决 AI Agent 领域术语混乱的问题。核心术语定义：
- **Model**：LLM 本身，无状态、无循环
- **Scaffolding**：行为定义层（系统提示、工具描述、上下文管理）
- **Harness**：执行层（调用模型、处理工具调用、决定何时停止）
- **Agent**：Model + 周围一切，使其能够自主行动
- **Context Engineering**：设计每一步进入上下文窗口的内容（系统提示、工具描述、对话历史、检索知识）
- **Skills**：可复用的结构化知识包，用于多步骤任务（区别于单一工具调用）

这篇术语表对 Agent 框架的理解和对比提供了统一的语言基础。

> 来源：[arXiv:2605.22794](https://arxiv.org/abs/2605.22794)、[arXiv:2605.22781](https://arxiv.org/abs/2605.22781)、[HuggingFace Blog](https://huggingface.co/blog/Dharma-AI/specialization-beats-scale)（2026-05-22）、[HuggingFace Agent Glossary](https://huggingface.co/blog/agent-glossary)（2026-05-25）

**Anthropic 完成 $650 亿 H 轮融资，估值 $965B**（2026-05-28）: Anthropic 宣布完成 650 亿美元 H 轮融资，由 Altimeter Capital、Dragoneer、Greenoaks 和 Sequoia Capital 领投，投后估值达 **9650 亿美元**。本轮还引入了 Micron、Samsung、SK hynix 等战略基础设施合作伙伴。Anthropic 运行收入已超过 470 亿美元/年。这反映了 Agent AI 市场需求的爆发式增长——企业正在将 Claude 部署到核心业务流程中。

**Google Antigravity：Agent-First 开发平台**（2026-05-20）: Google I/O 2026 发布的 Antigravity 是一个 Agent 优先的开发平台，提供 Agent 开发工具链（Agent Development Kit）、沙盒环境和运行时基础设施。配合 Gemini 3.5 Flash（以 Flash 级速度达到 Pro 级智能）和 Information Agents（在 Search 中创建可定制的 24/7 后台信息监控 Agent），Google 正在构建从模型到基础设施的完整 Agent 生态。

> 来源：[Anthropic Blog](https://www.anthropic.com/news/series-h)（2026-05-28）、[Google Blog](https://blog.google/innovation-and-ai/technology/ai/google-io-2026-all-our-announcements/)（2026-05-20）
...
**For Deeper Learning:**
- → **[5. Coding Agents](/docs/ai/agents/05-coding-agents)** - Claude Code, Devin, SWE-bench
- → **[6. Computer Use & GUI Agents](/docs/ai/agents/06-computer-use)** - Claude Computer Use, Operator

**For Production:**
- → **[9. Engineering & Production](/docs/ai/agents/09-engineering)** - Deployment, evaluation, security

**For Future:**
- → **[10. Frontier Trends](/docs/ai/agents/10-frontier)** - Emerging technologies
