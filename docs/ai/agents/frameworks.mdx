...
|| **AutoGen** | Python | Mature (0.4+)† | Excellent | Yes | Research ||
|| **CrewAI** | Python | Mature | Excellent | Yes | Role-based agents ||
|| **LangChain4j** | Java | Growing | Basic | Yes | Java port of LangChain |
|| **OpenAI Agents SDK** | Python | New | Good | Yes | OpenAI native agents |
|| **Google ADK** | Python | New | Good | Yes | Gemini-native agents |

> **† AutoGen 0.4+ Note**: AutoGen 0.4 introduced significant architectural changes from earlier versions, including a new async-first API and redesigned agent communication patterns. Teams upgrading from 0.2/0.3 should expect migration effort.

> **CrewAI**: Matured rapidly from "New" to "Mature" with the release of CrewAI Enterprise, providing production-grade features including observability, guardrails, and deployment tooling.
