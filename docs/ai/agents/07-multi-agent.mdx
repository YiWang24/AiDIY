...
### When More Is Less：多 Agent 协作何时失效（2026-09）

arXiv 2609.19759（*Rethinking Multi-Agent Collaboration: When More Is Less*）系统论证：随着单 Agent 能力（含 harness 质量）持续增强，多 Agent 协作并非总是带来真实增益。论文给出判别"何时多 Agent 有价值"的框架，并指出许多流行多 Agent 模式在强单体基线下收益为负。这与工程社区的实践观察相互印证——OpenAI Codex 工程师 Eric Provencher 曾警告"超过两个并行子 Agent 几乎总是在烧 token 而不提升质量"。

**选型启示**：在引入多 Agent 拓扑前，先跑强单 Agent + 良好 harness 的基线；多 Agent 的合理场景是任务天然可并行分解、子任务间信息依赖弱、以及需要独立视角交叉验证（如对抗性评审）时。并行度本身不是目标。

> 来源：[arXiv:2609.19759](https://arxiv.org/abs/2609.19759)（2026-09-17）

### Agensh：把组织智能扩展到 1024 个 Agent（2026-09-22）

arXiv 2609.26781（*Agensh: Scaling Organizational Intelligence to 1,024 Agents*）沿"Agent 社会"路线把规模推到四位数：通过分层组织结构让上千个 Agent 协同工作而不失控。这与"When More Is Less"的警告形成有趣对照——**规模本身既不是问题也不是答案，组织结构才是**：无结构的堆叠 Agent 数量会放大失败模式，而合适的分层与角色设计可以在大规模下保持增益。

与同期 2609.26780（SpeakerMem-R1）合看还有一层启示：多方（多 Agent）协作的记忆瓶颈与人类群聊一致——**谁说了什么、对谁说的、群体共享了什么**。SpeakerMem 的"说话人标签逐字消息 + 个人/群组双视图"设计对多 Agent 系统的共享记忆同样适用。

> 来源：[arXiv:2609.26781](https://arxiv.org/abs/2609.26781)、[arXiv:2609.26780](https://arxiv.org/abs/2609.26780)（2026-09-22）
