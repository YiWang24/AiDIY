...
| **Coding agent** | File-level context + Repo map + Procedural memory |
| **Computer use agent** | Screenshot buffer + Action history + Visual memory |

#### 2025 Memory Patterns

| Pattern | Description | Use Case |
|---------|-------------|----------|
| **Procedural Memory** | Store learned skills and workflows as reusable prompts | Agents that improve over repeated tasks |
| **Episodic Bundles** | Group related memories into task-level episodes | Complex multi-step task recall |
| **Cross-Agent Memory** | Shared memory with access control via A2A protocol | Multi-agent collaboration |
| **Compressed Long-Term** | LLM-generated summaries replacing raw memories | Cost-efficient long-running agents |
...
```mermaid
flowchart LR
    subgraph MCPv2["MCP v2 Architecture"]
        Agent[Agent] --> Client[MCP Client]
        Client --> S1[Filesystem<br/>Server]
        Client --> S2[GitHub<br/>Server]
        Client --> S3[Database<br/>Server]
        Client --> S4[Custom<br/>Server]
    end

    Client -->|Stream| Agent
    S1 -->|Tools/Resources| Client
    S2 -->|Tools/Resources| Client
    S3 -->|Tools/Resources| Client
    S4 -->|Tools/Resources| Client

    style MCPv2 fill:#f5f5f5
```
...
**For Implementation:**
- → **[3. Design Patterns](/docs/ai/agents/03-design-patterns)** - Learn proven patterns for structuring agents
- → **[4. Frameworks](/docs/ai/agents/04-frameworks)** - See how to implement with Spring AI

**For Advanced Topics:**
- → **[9. Engineering](/docs/ai/agents/09-engineering)** - Production deployment and evaluation
- → **[10. Frontier](/docs/ai/agents/10-frontier)** - Emerging trends and research
...
See **[4. Frameworks & Tech Stack](/docs/ai/agents/04-frameworks)** for complete Spring AI implementation examples of all these components.
:::
