...
Both OpenAI and Anthropic support prompt caching, offering significant cost savings for RAG systems:

**How it works**: When the same prompt prefix is sent repeatedly (e.g., system prompt + retrieved context template), the LLM provider caches the KV pairs. Subsequent requests with the same prefix only compute the new tokens.

| Provider | Min Tokens | Discount | TTL |
|----------|-----------|----------|-----|
| OpenAI | 1,024 | 50% off input | 5 min (extendable) |
| Anthropic | 1,024 (tool) / 2,048 (prompt) | 90% off input | 5 min |
...
**RAG Cost Impact**: For a system with a 4K system prompt + 8K retrieved context:
- Without caching: $0.015 per query (input tokens)
- With caching: $0.003 per query (80% savings)
- At 1M queries/day: ~$4,380/day savings

### Batch API

For non-real-time RAG workloads (document processing, evaluation, bulk indexing):
...
- Perfect for: document indexing, evaluation runs, bulk embeddings
...
**Next Steps**:
- 📖 Review [Advanced RAG Techniques](/docs/ai/rag/advanced-rag) for optimization strategies
