...
Anthropic 提出的 Contextual Retrieval 方法显著提升了检索质量：

**核心思想**: 在每个 chunk 前添加文档级上下文，使 embedding 和 BM25 索引更准确。
...
**效果**: 在多个基准测试中，Contextual Retrieval 将检索失败率降低了 49%（combined with BM25）到 67%（combined with embeddings）。

### LazyGraphRAG (Microsoft, 2024)

LazyGraphRAG 是 GraphRAG 的轻量级替代方案，大幅降低成本：

- **成本降低 700x+**: 不需要昂贵的 LLM 抽取，使用 NLP 技术进行实体和关系抽取
- **查询时图构建**: 在查询时才构建相关子图，而非预先构建全图
- **适合场景**: 中小规模文档集、预算受限的项目

### LightRAG

LightRAG 是另一个轻量级图检索方案：

- **双层检索**: 同时支持具体事实查询和抽象概念查询
- **增量更新**: 支持新文档增量插入，无需重建索引
- **更低资源需求**: 相比 GraphRAG 显著降低计算和存储需求

### Multimodal RAG

检索和生成不仅限于文本，还支持图片、表格、文档版面：
...
将 Agent 的自主决策能力融入 RAG 流程：

```mermaid
graph TD
    Q[User Query] --> A[RAG Agent]
    A -->|decide| R1[Vector Search]
    A -->|decide| R2[Web Search]
    A -->|decide| R3[SQL Query]
    A -->|decide| R4[Knowledge Graph]
    R1 --> A
    R2 --> A
    R3 --> A
    R4 --> A
    A -->|enough info?| G[Generate Answer]
    A -->|need more| A
```

**核心能力**:
- **自适应检索**: Agent 根据查询类型自动选择检索策略
- **迭代检索**: 如果首次检索结果不够，Agent 可以改写查询重新检索
- **多源融合**: 同时使用向量搜索、关键词搜索、知识图谱、SQL查询
- **自我评估**: Agent 判断检索结果是否足以回答问题

:::tip 2026 年 4月前沿：从检索到导航
**CorpusGraph**（[arXiv:2604.14572](https://arxiv.org/abs/2604.14572)）提出了一种全新的 Agentic RAG 范式：用 Agent **主动导航**企业知识语料库，而非被动检索。传统 RAG 系统无法回溯或组合分散的证据片段，CorpusGraph 让 Agent 学习组织结构，像人类研究员一样在文档间跳转、回溯、整合信息。这一思路正在推动 Agentic RAG 从"检索增强"向"知识导航"演进。

**UniDoc-RL**（[arXiv:2604.14967](https://arxiv.org/abs/2604.14967)）则将 RL 引入视觉 RAG，通过分层动作和密集奖励解决细粒度视觉语义的复杂推理问题，将外部视觉知识融入大视觉语言模型。
:::
...
### RAG 上下文剪枝：LLM 驱动的检索块过滤（2026）

传统 RAG 管线中，检索器以最大召回为目标，生成器（最昂贵的大模型）被迫阅读并忽略大部分不相关上下文。由于检索块在查询成本中占比约 **2/3**，每减少一个块可降低约 4% 查询成本。

**kapa.ai 的实践**（2026 年 7 月）表明，在 reranker 和生成器之间引入一个小型 LLM 进行 **listwise 五级评分**，可在不损失召回的前提下大幅压缩上下文：

| 评分 | 级别 | 含义 |
|------|------|------|
| 5 | ESSENTIAL | 答案必须依赖此块 |
| 4 | CONTRIBUTING | 不能单独作答，但与其他块组合必需 |
| 3 | SUPPORTING | 相关但答案可能完整无需 |
| 2 | TANGENTIAL | 同领域但无实质贡献 |
| 1 | UNRELATED | 无意义关联 |

**效果**：丢弃 **68%** 的上下文，保留 **96%** 的召回，查询净成本降低 **1/3**。

**关键洞察**：单个 chunk 的相关性不是一个独立属性，而取决于它与其他 chunk 的**组合关系**。pointwise cross-encoder reranker 对每个 query-chunk 对独立打分，无法识别"单独看无关、组合后必需"的块。锚点文档（anchor documents）方法虽能校准分数刻度，但同样无法解决此根本问题。因此，有效的剪枝器必须同时看到问题和所有 chunk，对整个集合进行判断。

> 来源：[kapa.ai](https://www.kapa.ai/blog/how-we-prune-rag-context)（2026-07）

### 多跳检索的失败是可预测的：置信度评分与主动弃答（2026-09）

传统 RAG 系统对所有查询一视同仁地给出答案，但 arXiv 2609.22056 证明：**多跳检索的失败并非均匀分布，而是聚集在结构上可预测的子群中**。论文给出两条形式化结果：

1. **CWAR 可归约性**：当且仅当失败查询的"自信-失败"结构满足特定条件时，confident-failure（自信但出错）才是可归约的；
2. **分数分布置信度评分**：仅用检索器的分数分布（无需额外模型调用）即可刻画查询的失败风险，识别出"模型自信但大概率会错"的查询。

**工程意义**：这为生产级 RAG 引入**弃答（abstention）机制**提供了理论基础——不是所有查询都值得生成，对高失败风险子群应触发降级策略（换检索策略、追问澄清、或明确告知用户"不确定"），而不是让模型在多跳链条上错上加错。与上述 kapa.ai 的上下文剪枝互补：剪枝解决"读什么"，置信度弃答解决"要不要答"。

> 来源：[arXiv:2609.22056](https://arxiv.org/abs/2609.22056)（2026-09-18）
...
**Next Steps**:
- 📖 Review [RAG Fundamentals](/docs/ai/rag/introduction) for system architecture
- 📖 Study [Evaluation Strategies](/docs/ai/rag/evaluation) to measure improvements
