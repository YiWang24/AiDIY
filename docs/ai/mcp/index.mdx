...
### MCP 生态里程碑与规范演进（2026-09）

2026 年 9 月，MCP 月下载量突破 **4 亿次**（一年内增长四倍），同期发布了该协议历史上最重要的规范更新之一：

- **传输模型简化**：从常驻双向连接转向**请求-响应模型**，MCP Server 从此可以部署在 Serverless 与边缘基础设施上，无需维持 7×24 常驻进程——大幅降低长尾工具的托管成本
- **交互式工具框架**：为需要多轮交互的工具（如确认、表单填写）提供形式化框架
- **企业身份集成**：收紧与 Microsoft Entra、Okta 等企业身份提供商（IdP）的对接规范，强化企业级认证治理
- **竞争者跟进**：OpenAI 与 Google 均已在自家产品中支持 MCP，协议事实上成为 Agent 连接外部工具的行业标准

> 来源：[unrot.co AI News](https://unrot.co/blogs/today-top-ai-news-september-1-2026)（2026-09-01）
...
:::info May 2026: Docker MCP Catalogs & AI Governance
...
> 来源：[Docker Blog](https://www.docker.com/blog/create-custom-mcp-catalogs-and-profiles/)（May 15, 2026）
:::

:::info MCP 2026 Roadmap (March 2026)
The MCP maintainer team published their [2026 roadmap](https://modelcontextprotocol.io/development/roadmap), targeting four priority areas for production readiness:
...
:::info May 2026: Qwen3.7-Max MCP 基准领先
...
:::info 2026-07-28: MCP 最大修订——无状态协议核心正式发布
MCP 维护团队正式发布了 `2026-07-28` 版本规范，这是自协议诞生以来的最大规模修订。核心变更包括：

- **无状态协议核心**：移除了协议层会话和 `Mcp-Session-Id` 头，取消了 `initialize`/`initialized` 握手流程，改为基于每请求的协议版本号和新的 `server/discover` RPC。这意味着任何请求可由负载均衡器后的任意服务器实例响应，彻底消除了粘性路由（sticky routing）需求
- **扩展框架（Extensions）**：引入正式的扩展机制，包括 MCP Apps（服务器端渲染 UI）和 Tasks（长时间运行任务的扩展）
- **授权强化**：更紧密地对齐 OAuth 和 OpenID Connect 部署
- **正式弃用策略**：旧的 HTTP+SSE 传输被重新分类为"已弃用"，Roots/Sampling/Logging 也被标记弃用
- **6 个 SEP 协同**：六份规范增强提案（SEP）共同实现了这一无状态架构

这一修订直接影响生产部署：此前水平扩展 MCP 服务器需要共享会话存储和粘性路由的复杂方案，现在任何标准 HTTP 基础设施都能直接处理请求分发。
