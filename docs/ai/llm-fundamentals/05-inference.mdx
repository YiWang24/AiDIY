...
### 2026: Gemma 4 Multi-Token Prediction (MTP)

**Gemma 4** (Google, 2026) 采用 **Multi-Token Prediction (MTP)** 作为推理加速的核心技术，实现最高 **3x 推理加速**。与传统 speculative decoding 使用独立 draft model 不同，Gemma 4 的 MTP drafter 直接集成在模型架构中。

**核心原理：**

| 技术 | 传统 Speculative Decoding | Gemma 4 MTP |
|------|--------------------------|-------------|
| Draft 模型 | 独立的小模型 | 模型内置 MTP head |
| 额外参数 | 完整 draft model | 轻量级线性层 |
| 推理开销 | 需要两次前向传播 | 单次前向传播 + 多头预测 |
| 加速比 | 2-3x | 最高 3x |

**MTP Drafter 工作流程：**

1. **训练阶段**：主模型额外训练多个 prediction head，每个 head 预测未来第 N 个 token
2. **推理阶段**：主模型生成当前 token 的同时，MTP heads 并行预测后续 token
3. **验证阶段**：使用 tree attention 一次性验证所有候选 token
4. **接受/拒绝**：通过 acceptance criterion 决定接受多少 token

**与 Medusa 的关键区别：**
- Medusa 的 heads 是事后添加的线性层，MTP heads 与主模型联合训练
- MTP 利用模型内部隐状态的更深层次信息
- 在长序列场景下优势更明显
...
### 2026: 异步 Continuous Batching

Hugging Face 于 2026 年 5 月发布 **异步 Continuous Batching** 技术，将 LLM 推理的 GPU 利用率从 ~75% 提升至接近 100%。

**核心原理：**

传统 Continuous Batching 中，CPU 准备下一个 batch（tokenization、采样）与 GPU 推理是串行的，约 25% 的 GPU 时间浪费在空闲间隙上。异步 batching 将 CPU batch 准备与 GPU 计算解耦并行：

| 对比维度 | 同步 Batching | 异步 Batching |
|---------|-------------|-------------|
| CPU/GPU 关系 | 串行（CPU 准备 → GPU 计算） | 并行（CPU 准备 ∥ GPU 计算） |
| GPU 利用率 | ~75%（存在空闲间隙） | 接近 100% |
| 延迟 | 较高 | 降低 |
| 吞吐量 | 基准 | 提升 ~30% |

**技术实现：**
- CPU 端独立线程处理 prefill 请求的 tokenization 和 KV cache 分配
- GPU 端持续执行 decode 步骤，不被 CPU 操作阻塞
- 双缓冲区机制确保数据一致性和无锁交换

> 来源：[Hugging Face Blog - Unlocking Asynchronicity in Continuous Batching](https://huggingface.co/blog/continuous_async)（May 14, 2026）
