...
### 2025–2026: Embedding Dimensions by Model
...
| **Gemini Embedding 2** | 可变 | 多模态（文本/图像/视频/音频），2026年4月 GA；2026年5月更新支持 interleaved 输入 |
...
| **Granite Embedding Multilingual R2** | 768/1024 | 多语言文本/代码，Apache 2.0，200+ 语言，32K 上下文 |

> **2026年4月更新**：Google 发布 **Gemini Embedding 2** GA 版本，这是首个原生多模态嵌入模型，支持文本、图像、视频和音频的统一向量表示。通过 Gemini API 和 Gemini Enterprise Agent Platform 提供，适用于电商搜索、视频分析等跨模态检索场景。

> **2026年5月更新**：arXiv:2605.27295 发布 **Gemini Embedding 2** 论文，详细披露该模型可嵌入视频、音频、图像和文本到统一表示空间，支持任意交叉模态的 interleaved 输入组合。这标志着嵌入模型从单模态文本向全模态统一表示的重要跨越。

> **2026年5月更新**：IBM 发布 **Granite Embedding Multilingual R2**（Apache 2.0），基于 ModernBERT 架构的两个开源多语言嵌入模型——97M 紧凑版（MTEB Multilingual Retrieval 60.3，百 M 以下最佳）和 311M 标准版（MTEB 65.2）。支持 200+ 语言、32K token 上下文（比 R1 扩展 64 倍）、9 种编程语言的代码检索，以及 Matryoshka 维度截断。此模型填补了开源高质量多语言嵌入模型的空白。[Hugging Face Blog](https://huggingface.co/blog/ibm-granite/granite-embedding-multilingual-r2)（May 14, 2026）
