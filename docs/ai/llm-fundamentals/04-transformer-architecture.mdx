...
### 2026: EMO — Emergent Modularity via End-to-End MoE Pretraining

**EMO**（Ai2, May 2026）代表了 MoE 架构的新范式：让模块化结构从数据中自然涌现，而非人工设计路由规则。

**核心创新：**
- **端到端预训练**：整个 MoE 结构（包括路由和专家）从头联合训练，无需预定义专家分工
- **涌现式模块化**：模型自动学会将不同能力分配给不同专家，形成可解释的功能专门化
- **高效推理**：每个任务仅需激活 12.5% 的专家，同时保持接近全量模型的性能
- **完全开源**：模型权重、代码和可视化工具全部公开
...
|------|------------|-----|
| 专家数量 | 256 | 可变（涌现决定） |
| 路由机制 | 预定义 Top-K | 端到端学习 |
| 模块化来源 | 人工设计 | 从数据中涌现 |
| 开源程度 | 模型权重 | 全栈（模型+代码+可视化） |

> 来源：[Hugging Face Blog - EMO](https://huggingface.co/blog/allenai/emo)（May 8, 2026）
...
- **Gated DeltaNet-2**（2026）: 通过解耦线性注意力中的擦除（erase）和写入（write）操作，解决了线性注意力在长上下文中的信息保留问题。Qwen3-Next 已采用 Gated DeltaNets 架构
- **Nemotron-Labs-Diffusion**（2026, NVIDIA）: 三模态语言模型，统一 AR、扩散（Diffusion）和自推测（Self-Speculation）解码。8B 版本每次前向传播解码 **5.9× 更多 token**（对比 Qwen3-8B），在 B200 上自推测模式达到约 **865 tok/s**（约 4× AR 基线），同时保持无损输出。这标志着扩散语言模型从研究走向实用化。

> 来源：[NVIDIA Research](https://research.nvidia.com/publication/2026-05_nemotron-labs-diffusion-tri-mode-language-model-unifying-autoregressive)（2026-05）
...
public class ModelSelectionService {
    ...
    public String chooseModel(String task) {
        return switch (task.toLowerCase()) {
            ...
            case "analysis", "document", "long" -> "gpt-4o",  // 128K context
...
11. **2026 "Language Models Need Sleep"**: CMU/Maryland 团队提出类睡眠的上下文整合机制——Transformer 在长上下文任务中，定期将 KV Cache 压缩为"快速权重（Fast Weight）"写入模型状态空间，类比人类睡眠中的记忆巩固。这项来自 [arXiv:2605.26099](https://arxiv.org/abs/2605.26099) 的研究为解决注意力机制的二次扩展问题提供了新思路。
