...
```mermaid
graph TD
    Start["Start: Understand Requirements"]

    step1["Step 1: Identify Constraints"]
    question1["What are we estimating?"]

    step2["Step 2: Gather Key Metrics"]
    question2["What metrics matter?"]

    step3["Step 3: Make Reasonable Assumptions"]
    question3["What are reasonable assumptions?"]

    step4["Step 4: Calculate Core Metrics"]
    question4["Calculate QPS, Storage, Bandwidth"]

    step5["Step 5: Identify Bottlenecks"]
    question5["What limits scale first?"]

    step6["Step 6: Add Safety Margin"]
    question6["Multiply by 2-5x for buffer"]

    End["Present Estimate with Confidence Range"]

    Start --> step1
    step1 --> question1
    question1 -->|"Traffic"| step2a["Users, DAU, MAU"]
    question1 -->|"Data"| step2b["Object size, count"]
    question1 -->|"Performance"| step2c["Latency, QPS"]
    ...
    step2 --> question2
    question2 -->|"Assumptions"| step3

    step3 --> question3
    question3 -->|"Assumption examples"| step3a["Peak = 3x avg<br/>Read:Write = 10:1<br/>Growth = 50%/year"]

    step3a --> step4
    step4 --> question4
    question4 -->|"Calculations"| step4a["QPS = Users × Requests / 86400<br/>Storage = Count × Size × Replication<br/>Bandwidth = QPS × (Req+Resp Size)"]

    step4a --> step5
    step5 --> question5
    question5 -->|"Bottlenecks"| step5a["Database QPS limit<br/>Network bandwidth<br/>Storage capacity<br/>Cache hit ratio"]

    step5a --> step6
    step6 --> question6
    question6 -->|"Add margin"| step6a["Average × 3 for peak<br/>Estimate × 2 for safety<br/>Plan 2-3 years growth"]
...
graph TB
    subgraph Users["User Base"]
        Registered["100M Registered Users"]
        DAU["20M Daily Active Users<br/>(20% of registered)"]
    end

    subgraph Traffic["Traffic Estimation"]
        DailyReqs["200M Requests/Day<br/>(20M × 10 req/user)"]
        AvgQPS["2,300 Average QPS<br/>(200M / 86,400s)"]
        PeakQPS["7,000 Peak QPS<br/>(2,300 × 3x peak multiplier)"]
    end

    subgraph Capacity["System Capacity Required"]
        API["API Servers<br/>(handles 7K QPS)"]
        DB["Primary Database<br/>(read replicas for scale)"]
        Cache["Cache Layer<br/>(Redis: 80% hit rate)"]
        Storage["Storage<br/>(tweet data, user profiles)"]
    end

    subgraph Bandwidth["Bandwidth Requirements"]
        ReadBW["Read: ~14 MB/s<br/>(9 reads × 2 KB × 7K QPS)"]
        WriteBW["Write: ~0.7 MB/s<br/>(1 write × 0.1 KB × 7K QPS)"]
        TotalBW["Total: ~15 MB/s<br/>(≈120 Mbps + overhead)"]
    end
