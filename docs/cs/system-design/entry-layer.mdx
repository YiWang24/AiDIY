...
sequenceDiagram
    ...
    participant Resolver as Local DNS Resolver<br/>(ISP/public DNS)
    participant Root as Root Server
    participant TLD as TLD Server
    participant Auth as Authoritative DNS<br/>(Route53/Cloudflare)
...
graph TB
    ...
    subgraph Regions["Regional Data Centers"]
        EU_DC["Europe Region<br/>Frankfurt"]
        US_DC["US Region<br/>Virginia"]
        Asia_DC["Asia Region<br/>Tokyo"]
    end
    ...
    Provider -->|"Route to nearest<br/>Based on resolver IP"| EU_DC
    Provider -->|"Route to nearest<br/>Based on resolver IP"| US_DC
    Provider -->|"Route to nearest<br/>Based on resolver IP"| Asia_DC

    EU_DC -.->|"Failover if<br/>unhealthy"| US_DC
    US_DC -.->|"Failover if<br/>unhealthy"| EU_DC
    Asia_DC -.->|"Failover if<br/>unhealthy"| US_DC
...
graph TB
    ...
    subgraph Edge["Edge Layer (Global POPs)"]
        Edge1["Edge Node 1<br/>New York"]
        Edge2["Edge Node 2<br/>London"]
        Edge3["Edge Node 3<br/>Tokyo"]
    end

    subgraph Regional["Regional Layer (Mid-Level Caches)"]
        Regional1["Regional Cache 1<br/>US East"]
        Regional2["Regional Cache 2<br/>Europe"]
        Regional3["Regional Cache 3<br/>Asia Pacific"]
    end

    subgraph Origin["Origin Infrastructure"]
        CDN["CDN Origin Shield<br/>Central Cache"]
        OriginServer["Origin Server<br/>Your Application"]
    end

    User -->|"1. Request"| Edge1

    Edge1 -->|"2. Cache Miss<br/>Look for content"| Regional1
    Edge2 -->|"2. Cache Miss"| Regional2
    Edge3 -->|"2. Cache Miss"| Regional3

    Regional1 -->|"3. Cache Miss<br/>Aggregate requests"| CDN
    Regional2 -->|"3. Cache Miss"| CDN
    Regional3 -->|"3. Cache Miss"| CDN

    CDN -->|"4. Cache Miss<br/>Last resort"| OriginServer
    OriginServer -->|"5. Response"| CDN
    CDN -->|"6. Cache & forward"| Regional1
    Regional1 -->|"7. Cache & forward"| Edge1
    Edge1 -->|"8. Deliver to user<br/>< 50ms latency"| User
