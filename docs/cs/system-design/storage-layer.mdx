...
```mermaid
graph TD
    Start["What are your requirements?"] --> Q1{"Need complex<br/>joins and<br/>relations?"}

    Q1 -->|Yes| Relational["Relational Database<br/>(PostgreSQL, MySQL)"]
    Q1 -->|No| Q2{"Need flexible<br/>schema or<br/>rapid evolution?"}

    Q2 -->|Yes| Document["Document Database<br/>(MongoDB, Couchbase)"]
    Q2 -->|No| Q3{"Primary access<br/>by key only?"}

    Q3 -->|Yes| Q4{"Need sub-millisecond<br/>latency or caching?"}
    Q3 -->|No| Q5{"Heavy write<br/>workload or<br/>time-series data?"}

    Q4 -->|Yes| KV["Key-Value Store<br/>(Redis, DynamoDB)"]
    Q4 -->|No| WideColumn["Wide-Column Store<br/>(Cassandra, ScyllaDB)"]

    Q5 -->|Yes| Column["Column-Family Store<br/>(Cassandra, ScyllaDB)"]
    Q5 -->|No| Q6{"Need full-text<br/>search or<br/>complex aggregations?"}

    Q6 -->|Yes| Search["Search Engine<br/>(Elasticsearch)"]
    Q6 -->|No| Q7{"Large files<br/>or blob storage?"}

    Q7 -->|Yes| Object["Object Storage<br/>(S3, GCS, Azure Blob)"]
    Q7 -->|No| Q8{"Time-series<br/>data?"}

    Q8 -->|Yes| TS["Time-Series DB<br/>(InfluxDB, TimescaleDB)"]
    Q8 -->|No| Default["Relational Database<br/>(PostgreSQL, MySQL)"]

    Relational --> R1["Use cases:<br/>- Financial transactions<br/>- Complex queries<br/>- Strong consistency"]
    Document --> D1["Use cases:<br/>- Content management<br/>- Product catalogs<br/>- User profiles"]
    KV --> K1["Use cases:<br/>- Sessions/caching<br/>- Real-time leaderboards<br/>- Shopping carts"]
    WideColumn --> WC1["Use cases:<br/>- High write volume<br/>- Time-series data<br/>- Multi-region"]
    Column --> C1["Use cases:<br/>- IoT sensors<br/>- Metrics/events<br/>- Log data"]
    Search --> S1["Use cases:<br/>- Product search<br/>- Log analytics<br/>- Autocomplete"]
    Object --> O1["Use cases:<br/>- Media files<br/>- Backups<br/>- Data lakes"]
    TS --> T1["Use cases:<br/>- Monitoring metrics<br/>- IoT data<br/>- Market data"]
...
graph TB
    subgraph CAP["CAP Theorem Triangle"]
        C["Consistency (C)<br/>Every read receives<br/>the most recent write<br/>or an error"]
        A["Availability (A)<br/>Every request receives<br/>a response, without<br/>guarantee of latest write"]
        P["Partition Tolerance (P)<br/>System continues to operate<br/>despite network failures"]
    end

    C --- P
    A --- P
    C -.-"Must choose<br/>during partition"-.-> A

    subgraph Systems["Trade-offs in Practice"]
        CP["CP Systems<br/>Choose Consistency<br/>Returns errors<br/>during partitions<br/>Examples: HBase,<br/>MongoDB (strong)"]
        AP["AP Systems<br/>Choose Availability<br/>May serve stale data<br/>Examples: Cassandra,<br/>DynamoDB"]
        CA["CA Systems<br/>Single-node only<br/>Not truly distributed<br/>Examples: PostgreSQL<br/>single instance"]
    end
...
sequenceDiagram
    ...
    participant Leader as Primary Node<br/>(Leader)
