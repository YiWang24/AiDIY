...
graph TB
    ...
    subgraph L1["Level 1: Client-Side Cache (Browser/App)"]
        ClientCache["Browser Cache: <1ms latency, Limited capacity, User-controlled"]
    end

    subgraph L2["Level 2: CDN/Edge Cache (Global POPs)"]
        Edge["Edge CDN: 10-50ms latency, Global distribution, Vendor-managed"]
    end

    subgraph L3["Level 3: Application Cache (In-Process)"]
        AppCache["Local Cache: <0.1ms latency, Per-instance, Small capacity"]
    end

    subgraph L4["Level 4: Distributed Cache (Shared Cluster)"]
        Redis["Redis/Memcached: 1-10ms latency, Shared across instances, Large capacity"]
    end

    subgraph L5["Level 5: Database (Persistent Storage)"]
        DB["Database: 10-100ms latency, Persistent storage, Source of truth"]
    end
...
sequenceDiagram
    ...
    Note over App,DB: Every request hits DB. Database overwhelmed.
    ...
    rect rgb(240, 200, 200)
        ...
        App-->>Attacker: 3. Reject immediately (key doesn't exist)
