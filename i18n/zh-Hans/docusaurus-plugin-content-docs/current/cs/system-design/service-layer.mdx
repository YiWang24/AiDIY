...
graph TB
    ...
    subgraph Microservices["Microservices Architecture"]
        ...
        AuthSvc["Auth Service (Team: Auth)"]
        OrderSvc["Order Service (Team: Orders)"]
        PaySvc["Payment Service (Team: Payments)"]
        InvSvc["Inventory Service (Team: Inventory)"]
        NotifSvc["Notification Service (Team: Notifications)"]
...
stateDiagram-v2
    ...
    Closed --> Open: Failure Threshold Exceeded (e.g. 5 failures in 1 min)
    ...
    Open --> HalfOpen: Timeout Expires (e.g. 60 seconds)
